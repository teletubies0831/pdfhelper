...
export const AGENT_TOOL_DEFINITIONS: AgentToolDefinition[] = [
  ...
  {
    name: "document.readPages",
    apiName: "document_read_pages",
    label: "读取指定页",
  ...
  {
    name: "document.getProfile",
    apiName: "document_get_profile",
    label: "读取论文档案",
    description:
  ...
  {
    name: "document.getOutline",
    apiName: "document_get_outline",
    label: "读取 PDF 目录",
  ...
  {
    name: "document.inspectPageImage",
    apiName: "document_inspect_page_image",
    label: "查看 PDF 页面图像",
  ...
  },
  {
    ...
    trigger:
      "用户说“记住”，或明确表达稳定偏好、研究方向、个人资料、持续项目目标时。",
    parametersSummary:
    ...
    parameters: objectSchema(
      {
        key: {
          type: "string",
          description: "稳定英文点分 key，例如 profile.education.major。",
        },
        category: {
          type: "string",
        ...
        content: { type: "string" },
        scope: { type: "string", enum: ["global", "project", "pdf"] },
  ...
  },
  {
    ...
    apiName: "library_get_paper",
    label: "读取历史文献记录",
    description:
      "按文献 ID 读取一篇历史 PDF 的元数据、阅读记录、论文阅读卡片和关联的片段卡片。",
    trigger: "已经从文献库搜索得到目标文献，需要查看详情时。",
    parametersSummary: "documentId",
    parameters: objectSchema({ documentId: { type: "string" } }, [
      "documentId",
    ]),
  },
  {
    name: "library.readPaper",
    apiName: "library_read_paper",
    label: "读取历史论文原文",
    description:
      "通过历史文献记录中保存的本地文件句柄或远程地址，读取指定页或检索整篇论文原文。",
    trigger:
      "模型需要核对一篇历史论文的原文、具体方法、实验数据或结论，而阅读卡片信息不足时。",
...
export function getNativeAgentTools(
  ...
  return AGENT_TOOL_DEFINITIONS.filter(
    (tool) => !allowedNames || allowedNames.has(tool.name),
  ).map((tool) => ({
    type: "function",
    function: {
      name: tool.apiName,
