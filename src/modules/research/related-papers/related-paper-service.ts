...
import {
  ...
  titleKey,
  tokenize,
} from "../common/research-runtime";
import type { RawLiteratureCandidate } from "../common/research-runtime";
import {
  CCF_INDEX_URL,
  ccfOfficialDirectoryTool,
  loadCcfDirectory,
  readStorage,
} from "../ccf/ccf-service";
import {
  crossrefSearchTool,
  dblpSearchTool,
  openAlexSearchTool,
  semanticScholarExactTool,
  semanticScholarRecommendationsTool,
  semanticScholarSearchTool,
} from "../providers/literature-providers";
...
export function deduplicateCandidates(
  items: RawLiteratureCandidate[],
): RawLiteratureCandidate[] {
  const map = new Map<string, RawLiteratureCandidate>();
  for (const item of items) {
    const key = item.doi
      ? `doi:${item.doi.toLowerCase()}`
      : `title:${titleKey(item.title)}`;
    ...
    const preferred =
      scoreCandidateCompleteness(item) > scoreCandidateCompleteness(existing)
        ? item
        : existing;
    map.set(key, {
      ...existing,
      ...
      origin:
        existing.origin === "recommendation" || item.origin === "recommendation"
          ? "recommendation"
          : preferred.origin,
      region:
        existing.region === "domestic" || item.region === "domestic"
          ? "domestic"
          : "international",
...
export function scoreCandidateCompleteness(
  item: RawLiteratureCandidate,
): number {
  return (
    (item.abstract ? 3 : 0) +
    (item.venue ? 2 : 0) +
    (item.doi ? 2 : 0) +
    (item.url ? 1 : 0) +
    (item.citationCount > 0 ? 1 : 0) +
    (item.origin === "recommendation" ? 1 : 0)
  );
}
...
export function calculateRelevance(
  ...
  const candidateTokens = new Set(
    tokenize(
      `${candidate.title} ${candidate.abstract ?? ""} ${candidate.venue}`,
    ),
  );
  let overlap = 0;
  ...
  for (const phrase of queryPhrases) {
    if (phrase.length >= 8 && normalizedCandidate.includes(phrase))
      phraseMatches += 1;
  }
...
export function compareRelated(
  ...
  if (Math.abs(right.relevance - left.relevance) > 0.0001)
...
export function selectDiversePapers(sorted: RelatedPaper[]): RelatedPaper[] {
  ...
  const international = sorted.find(
    (paper) => paper.region === "international",
  );
  for (const desired of [domestic, international]) {
    if (
      !desired ||
      selected.some((paper) => paper.id === desired.id) ||
      selected.length === 0
    )
      continue;
    ...
    if (
      weakest &&
      desired.qualityScore >= weakest.qualityScore - 8 &&
      desired.relevance >= weakest.relevance - 0.18
    ) {
  ...
  return Array.from(
    new Map(selected.map((paper) => [paper.id, paper])).values(),
  )
    .sort(compareRelated)
    .slice(0, MAX_VISIBLE_PAPERS);
}
...
export const planQueriesNode = async (state: {
  request: RelatedResearchRequest;
}) => ({
  queries: buildFocusedQueries(state.request),
});

export const searchSourcesNode = async (state: {
  request: RelatedResearchRequest;
  queries: string[];
}) => {
  const exactRaw = await semanticScholarExactTool
    .invoke({ title: state.request.title })
    .catch(() => "[]");
  const exact = JSON.parse(String(exactRaw)) as RawLiteratureCandidate[];
  const exactPaper =
    exact.find(
      (paper) => titleKey(paper.title) === titleKey(state.request.title),
    ) ?? exact[0];
  ...
  if (exactPaper?.paperId) {
    tasks.push(
      semanticScholarRecommendationsTool
        .invoke({ paperId: exactPaper.paperId })
        .then(String)
        .catch(() => "[]"),
    );
  }
  ...
  for (const query of broadQueries.slice(0, 4)) {
    tasks.push(
      openAlexSearchTool
        .invoke({ query, domesticOnly: false })
        .then(String)
        .catch(() => "[]"),
    );
    tasks.push(
  ...
  for (const query of broadQueries.slice(0, 3)) {
    ...
    tasks.push(
      openAlexSearchTool
        .invoke({ query, domesticOnly: true })
        .then(String)
        .catch(() => "[]"),
    );
  }
  ...
  const all = [
    ...
    ...settled.flatMap((raw) => {
      try {
        return JSON.parse(raw) as RawLiteratureCandidate[];
      } catch {
        return [];
      }
    }),
...
export const verifyAndRankNode = async (state: {
  request: RelatedResearchRequest;
  candidates: RawLiteratureCandidate[];
}) => {
  ...
  const scored = state.candidates
    ...
    .map((candidate) => ({
      candidate,
      ...calculateRelevance(candidate, state.request),
    }))
    .filter(
      ({ candidate, relevance, overlap }) =>
        candidate.origin === "recommendation" ||
        relevance >= 0.14 ||
        overlap >= 2,
    )
  ...
  const papers = await Promise.all(
    scored.map(
      async ({ candidate, relevance, overlap }): Promise<RelatedPaper> => {
        ...
        if (candidate.venue) {
          try {
            ccfResult = JSON.parse(
              String(
                await ccfOfficialDirectoryTool.invoke({
                  venue: candidate.venue,
                  force: false,
                }),
              ),
...
export async function runRelatedResearchGraph(
  ...
  if (!request.force) {
    const cached = await readStorage<{
      savedAt: number;
      response: RelatedResearchResponse;
    }>(cacheKey);
