...
export async function buildDocumentRetrievalContext(
  ...
  const explicitlyReferencesPdfVisual =
    /(?:PDF|文档|论文|本页|当前页|第\s*\d+\s*页).{0,12}(?:图\s*\d*|图表|表格|公式|方程|曲线|流程图|架构图|示意图|figure|fig\.?|table|chart|equation|diagram)/i.test(
      options.question,
    ) ||
    /(?:figure|fig\.?|图)\s*\d+.{0,12}(?:PDF|文档|论文|本页|当前页|第\s*\d+\s*页)/i.test(
      options.question,
    );
  ...
  const userImageQuestion = Boolean(
    options.userImageAttached && !explicitlyReferencesPdfVisual,
  );
  ...
  for (let round = 1; round <= maximumPlanningRounds; round += 1) {
    ...
    try {
      ...
      const plan = await options.requestAi(
        [{ role: "user", content: planningPrompt }],
        {
          ...
          sourcePages: uniquePages([
            options.currentPage,
            ...toolResults.flatMap((result) => result.pages),
          ]),
          contextNote:
            "只判断证据是否充分并选择工具；必须返回 JSON，不直接回答用户。",
        },
      );
      const decision = parseDocumentToolDecision(plan);
      if (
        /Planner returned (?:no valid|malformed) JSON\./.test(decision.reason)
      ) {
      ...
      const calls = decision.calls
        .filter(
          (call) => !(userImageQuestion && call.name === "inspect_page_image"),
        )
  ...
  if (
    ...
    toolResults.push(
      ...(await executeDocumentToolCalls([fallbackCall], options)),
    );
  ...
  const retrievalText = toolResults.length
    ...
    : "";
  const currentPageLabel = userImageQuestion
    ? "PDF 当前页文字补充"
    : "当前页正文";
