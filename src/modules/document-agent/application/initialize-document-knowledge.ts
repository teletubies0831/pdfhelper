...
import type {
  InitializeDocumentKnowledgeOptions,
  InitializedDocumentKnowledge,
} from "./runtime-contracts";
...
export async function analyzePaperChunks(
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const summary = await options.requestAi(
      [
        {
          role: "user",
          content: strategy.buildChunkAnalysisPrompt(
      ...
      ],
      {
        ...
        sourcePages: Array.from(
          ...
          (_, offset) => chunk.startPage + offset,
        ),
  ...
  const synthesisSource = chunkSummaries
    .map((summary) => summary.slice(0, 1800))
    .join("\n\n")
    .slice(0, 24000);
...
export async function initializeDocumentKnowledge(
  ...
  if (canRestore && existingRecord) {
    ...
    options.onStatus({
      text: statusText,
      status: existingRecord.processingStatus,
    });
    return {
      documentId,
  ...
  record.processingStatus =
    strategy.autoAnalyzeWholeDocument && !options.hasApiKey
      ? "needs-api-key"
      : "indexed";
  record.updatedAt = Date.now();
  await putDocumentAgentRecord(record);
