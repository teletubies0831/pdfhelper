...
export async function executeDocumentToolCalls(
  ...
  const maximumPage =
    getDocumentAgentStrategy(options.readingMode).knowledgeBoundary ===
    "current-position"
      ? options.currentPage
      : undefined;
  ...
  for (const call of calls) {
    if (call.name === "search_document") {
      ...
      const matches = searchDocumentChunks(
        options.chunks,
        query,
        limit,
        maximumPage,
      ...
      results.push({
        ...
        label: `全文检索：${query}`,
        pages: uniquePages(
          matches.flatMap(({ chunk }) => [chunk.startPage, chunk.endPage]),
        ),
    ...
    if (call.name === "read_pages") {
      let startPage = Math.min(
        options.pageCount,
        toPositivePage(call.arguments.startPage, options.currentPage),
      );
      let endPage = Math.min(
        options.pageCount,
        toPositivePage(call.arguments.endPage, startPage),
      );
    ...
    if (call.name === "read_section") {
      const requestedTitle =
        toStringValue(call.arguments.title) ||
        detectRequestedSectionTitle(options.question, options.outline);
      ...
      if (!range) {
        const matches = searchDocumentChunks(
          options.chunks,
          requestedTitle || options.question,
          4,
          maximumPage,
        );
        results.push({
      ...
      results.push({
        ...
        pages: Array.from(
          { length: Math.max(0, allowedEndPage - range.startPage + 1) },
    ...
    if (call.name === "get_document_profile") {
      results.push({
        ...
        label: "整篇论文档案",
        pages: [],
        content: options.record?.profile
          ? profileToText(options.record.profile)
          : "整篇论文档案尚未生成。",
      });
      continue;
    }
      ...
      results.push({
        ...
        content: options.outline.length
          ? options.outline
              .map(
                (item) =>
                  `${"  ".repeat(item.depth)}- ${item.title}（第 ${item.pageNumber} 页）`,
              )
              .join("\n")
          : "该 PDF 没有可用目录。",
      });
    ...
    if (call.name === "inspect_page_image") {
      const pageNumber = Math.min(
        ...
        Math.min(
          options.pageCount,
          toPositivePage(call.arguments.pageNumber, options.currentPage),
        ),
      );
      const question =
        toStringValue(call.arguments.question) || options.question;
      if (!options.hasVisionModel || !options.inspectPageImage) {
        results.push({
          ...
          pages: [pageNumber],
          content:
            "视觉模型尚未配置，无法读取该页中的图、表、公式截图或空间布局。",
        });
        continue;
      }
      try {
