...
import type { DocumentAgentStrategy, DocumentChunk, DocumentPageText } from './contracts';



export const normalizeWhitespace = (value: string): string => value
  .replace(/\u00ad/g, '')
  .replace(/[\t ]+/g, ' ')
  .replace(/\s*\n\s*/g, '\n')
  .replace(/\n{3,}/g, '\n\n')
...
export function splitLongText(text: string, maxCharacters: number, overlapCharacters: number): string[] {
