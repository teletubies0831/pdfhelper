...
export const DOCUMENT_TOOL_DESCRIPTIONS = [
  {
    ...
    description:
      "Search the complete locally indexed document for relevant concepts, methods, evidence, or results.",
    arguments: {
      query: "Search question or keywords",
      limit: "Integer from 1 to 8; default 5",
    },
  },
  {
    name: "read_pages",
    ...
    arguments: {
      ...
      endPage: "Last page; at most 8 consecutive pages",
    },
  },
  {
    name: "read_section",
    description:
      "Read the complete section identified by its outline title, including continuation pages until the next peer section.",
    arguments: {
      title:
        "Section title, for example Introduction, Method, Experiments, or Conclusion",
    },
  },
  {
    name: "get_document_profile",
    description:
      "Read the whole-paper profile created during first-time processing.",
    arguments: {},
  },
  {
  ...
  },
  {
    ...
    description:
      "Visually inspect a PDF page when the question depends on a figure, chart, equation, table, diagram, image, or page layout that text extraction cannot preserve.",
    arguments: {
      pageNumber: "Page to inspect",
      question: "What visual information to inspect",
    },
  },
] as const;

export function buildDocumentToolPlanningPrompt(
  context: DocumentToolPlanningContext,
): string {
  const {
    question,
    currentPage,
  ...
  const tools = visionAvailable
    ...
    : DOCUMENT_TOOL_DESCRIPTIONS.filter(
        (tool) => tool.name !== "inspect_page_image",
      );
...
export function parseDocumentToolDecision(
  content: string,
): DocumentToolDecision {
  ...
  if (start < 0 || end < start) {
    return {
      decision: "answer",
      reason: "Planner returned no valid JSON.",
      calls: [],
    };
  }
  try {
    ...
    const calls = (Array.isArray(parsed.calls) ? parsed.calls : [])
      .filter(
        (call): call is { name: DocumentToolName; arguments?: unknown } =>
          Boolean(call) &&
          typeof call === "object" &&
          names.has(
            (call as { name?: DocumentToolName }).name as DocumentToolName,
          ),
      )
      .slice(0, 3)
      .map((call) => ({
    ...
    const decision =
      parsed.decision === "use_tools" && calls.length ? "use_tools" : "answer";
    return {
      decision,
      reason:
    ...
    return {
      decision: "answer",
      reason: "Planner returned malformed JSON.",
      calls: [],
    };
  }
}
...
export function formatDocumentToolResults(
  results: DocumentToolResult[],
): string {
  return results
    .map((result, index) =>
      [
        ...
        result.pages.length
          ? `Source pages: ${result.pages.join(", ")}`
