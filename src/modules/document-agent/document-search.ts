...
export function searchDocumentChunks(
  ...
  return chunks
    .filter(
      (chunk) => maximumPage === undefined || chunk.startPage <= maximumPage,
    )
    .map((chunk) => {
      const haystack =
        `${chunk.heading ?? ""}\n${chunk.summary ?? ""}\n${chunk.text}`
      ...
      for (const term of matchedTerms)
        score += Math.min(8, haystack.split(term).length - 1);
      if (normalizedQuery.length >= 4 && haystack.includes(normalizedQuery))
        score += 24;
      if (
        chunk.summary &&
        matchedTerms.some((term) => chunk.summary?.toLowerCase().includes(term))
      )
        score += 5;
