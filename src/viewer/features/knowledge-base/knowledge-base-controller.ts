...
import { aiPanelToggleButton, knowledgeBaseEntryButton, knowledgeBasePageElement, knowledgeDocumentCountElement, knowledgeGroupSelect, knowledgeListElement, knowledgePageSubtitleElement, knowledgePageTitleElement, knowledgeSearchInput, knowledgeSortSelect, knowledgeTotalCountElement } from "../../app/viewer-elements";
import {
...
export function openKnowledgeBasePage(): void {
  const openToken = ++knowledgeOpenToken;
  ...
  void transitionWorkspaceView("knowledge").then(() => {
    if (
      openToken !== knowledgeOpenToken ||
      getActiveWorkspaceView() !== "knowledge"
    ) {
      return;
    }
...
export function closeKnowledgeBasePage(): void {
  if (getActiveWorkspaceView() !== "knowledge") return;
  knowledgeOpenToken += 1;
