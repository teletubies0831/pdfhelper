...
import { isVisionAiConfigured, type AiImageAttachment, type AiRuntimeResponse, type AiNativeToolCall, type AiStreamStartMessage, type AiStreamToolResult } from "../../../../shared/ai";


import { getAgentToolDefinitionByApiName } from "../../../../shared/agent-tools";
import { type MemoryToolCall, type PaperLibraryRecord } from "../../../../shared/memory";
...
export function getPaperLibraryCardContext(record: PaperLibraryRecord): Record<string, unknown> {
...
export async function getHistoricalPaperBytes(record: PaperLibraryRecord): Promise<Uint8Array> {
...
export async function executeLibraryReadPaper(
  ...
  const isCurrentDocument = Boolean(
    pdfDocument.value && getDocumentChatId(pdfDocument.value) === record.documentId,
  );
    ...
    try {
      ...
      if (toolName === "journal.search") {
        ...
        return {
          ...
          name: toolName,
          ok: true,
          content: JSON.stringify({ readingMode: resolvedReadingMode.value, entries }, null, 2),
        };
      }
