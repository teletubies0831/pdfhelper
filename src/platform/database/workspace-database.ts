...
export function openDocumentAgentDatabase(): Promise<IDBDatabase> {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open(
      DOCUMENT_AGENT_DB_NAME,
      DOCUMENT_AGENT_DB_VERSION,
    );
    request.onupgradeneeded = () => {
      ...
      if (visionCache)
        ensureIndex(visionCache, DOCUMENT_ID_INDEX, DOCUMENT_ID_INDEX);

      const memories = database.objectStoreNames.contains(
        LONG_TERM_MEMORIES_STORE,
      )
        ? transaction?.objectStore(LONG_TERM_MEMORIES_STORE)
        : database.createObjectStore(LONG_TERM_MEMORIES_STORE, {
            keyPath: "id",
          });
      if (memories) {
      ...
      const paperLibrary = database.objectStoreNames.contains(
        PAPER_LIBRARY_STORE,
      )
        ? transaction?.objectStore(PAPER_LIBRARY_STORE)
        : database.createObjectStore(PAPER_LIBRARY_STORE, { keyPath: "id" });
      if (paperLibrary) {
        ensureIndex(paperLibrary, FINGERPRINT_INDEX, FINGERPRINT_INDEX, {
          unique: false,
        });
    ...
    request.onerror = () =>
      reject(
        request.error ?? new Error("Unable to open document agent database."),
      );
    request.onblocked = () =>
      reject(
        new Error(
          "数据库升级被其他已打开的 PDFPal 页面阻塞，请关闭旧页面后重试。",
        ),
      );
  });
}
