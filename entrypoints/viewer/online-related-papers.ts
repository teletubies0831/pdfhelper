import { renderCcfRankResult } from "./ccf-rank";
import { runRelatedResearch, type RelatedPaper } from "./research-tools";
...
function getSection(): { section: HTMLElement; result: HTMLElement } | null {
  ...
  const section = legacy.closest<HTMLElement>(".paper-card-side-card") ??
    legacy.closest<HTMLElement>("section");
  if (!section) return null;

  section.classList.add("online-related-paper-section");
  legacy.closest<HTMLElement>("label")?.setAttribute("hidden", "");
